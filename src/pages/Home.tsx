...
export default function Home() {
  ...
  // Филтрирани профили по пребарување
  ...
  // Само категории кои имаат профили (по филтер)
  const activeCategories = mockCategories.filter(cat =>
    filteredProfiles.some(p => p.categorySlug === cat.slug)
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mb-12">
          <div className="relative max-w-2xl mx-auto">
            ...
            <input
              ...
              placeholder="Пребарај бизниси, категории, локации..."
              className="w-full pl-12 pr-12 py-4 rounded-2xl border border-slate-200 bg-white shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none text-slate-800 placeholder-slate-400 text-base transition-all"
            />
            {searchQuery && (
              <button
          ...
          {query && (
            <p className="text-center text-sm text-slate-500 mt-3">
              {filteredProfiles.length === 0
                ...
                : `Пронајдени ${filteredProfiles.length} профил${filteredProfiles.length === 1 ? '' : 'и'} за „${searchQuery}"`}
            </p>
          )}
        </div>
        ...
        <div className="space-y-20 flex flex-col w-full">
          ...
          {query && filteredProfiles.length > 0 && (
            <section>
              <h2 className="text-2xl font-extrabold text-slate-900 mb-8 border-b border-slate-300 pb-4">Резултати</h2>
