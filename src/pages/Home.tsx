...
export default function Home() {
  const { categories: mockCategories, profiles: mockProfiles, articles: mockArticles } = useData();
  
  // Only display categories that actually have profiles in our mock data
  const activeCategories = mockCategories.filter(cat => 
    mockProfiles.some(p => p.categorySlug === cat.slug)
  );
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {/* Главна содржина полна ширина */}
        <div className="space-y-20 flex flex-col w-full">
          
          {/* Iterated Categories with Featured Profiles */}
          <div className="space-y-16">
            {activeCategories.map(category => {
              const categoryProfiles = mockProfiles.filter(p => p.categorySlug === category.slug).slice(0, 4); // show up to 4 per category on home page
              return (
